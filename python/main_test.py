from fastapi.testclient import TestClient
from main import app, get_db
from fastapi.testclient import TestClient
import pytest
import sqlite3
import os
import pathlib

client = TestClient(app)

# STEP 6-4: uncomment this test setup
test_db = pathlib.Path(__file__).parent.resolve() / "db" / "test_mercari.sqlite3"

def override_get_db():
    conn = sqlite3.connect(test_db, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    try:
        yield conn
    finally:
        conn.close()

app.dependency_overrides[get_db] = override_get_db

@pytest.fixture(autouse=True)
def db_connection():
    # Create a test database
    conn = sqlite3.connect(test_db, check_same_thread=False)
    cursor = conn.cursor()
    
    # Create `categories` table
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS categories (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL UNIQUE
    )
    """)

    # Create `items` table
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS items (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        category_id INTEGER NOT NULL,
        image_name TEXT NOT NULL,
        FOREIGN KEY (category_id) REFERENCES categories(id)
    )
    """)

    conn.commit()
    conn.row_factory = sqlite3.Row  # Return rows as dictionaries

    yield conn  # Provide connection to test

    conn.close()
    
    # After test, remove test database
    if test_db.exists():
        test_db.unlink()  # Delete test DB file

@pytest.mark.parametrize(
    "want_status_code, want_body",
    [
        (200, {"message": "Hello, world!"}),
    ],
)
def test_hello(want_status_code, want_body):
    response=client.get("/")
    response_body = response.json()
    # STEP 6-2: confirm the status code
    assert response.status_code==want_status_code, f"Expected {want_status_code}, but got {response.status_code}"
    # STEP 6-2: confirm response body
    assert response_body==want_body, f"Expected {want_body}, but got {response_body}"

# STEP 6-4: uncomment this test
@pytest.mark.parametrize(
    "args, want_status_code",
        [
        ({"name":"used iPhone 16e", "category":"phone"}, 200),
        ({"name":"", "category":"phone"}, 400),
    ],
)
def test_add_item_e2e(args,want_status_code,db_connection):
    with TestClient(app) as client:
        with open("test_image.jpg", "wb") as f:
            f.write(b"test image content")  # ダミー画像データ作成

        with open("test_image.jpg", "rb") as img_file:
            response = client.post(
                "/items/",
                data=args,  # `json=` ではなく `data=`
                files={"image": img_file}  # `image` を `files` で送る
            )
    
        print(response.json())  # API のレスポンスを確認
    
        assert response.status_code == want_status_code
    
        if want_status_code >= 400:
            response_data = response.json()
            assert "detail" in response_data, "Error response should contain 'detail' field"
            return 
    
    
        # Check if the response body is correct
        response_data = response.json()
        assert response_data["message"] == "Item added successfully", f"Unexpected message: {response_data['message']}"
        # assert "message" in response_data

        # Check if the data was saved to the database correctly
        cursor = db_connection.cursor()
        cursor.execute("SELECT * FROM items WHERE name = ?", (args["name"],))
        db_item = cursor.fetchone()
        assert db_item is not None, "Item was not found in the database"
        assert dict(db_item)["name"] == args["name"]
